...
class Setting extends Component
{
    ...
    {
        if (!yii::$app->option->website_status) {
            yii::$app->catchAll = ['site/offline'];
        }
        yii::$app->language = yii::$app->option->website_language;
        yii::$app->timeZone = yii::$app->option->website_timezone;
    ...
    {
        if (!empty(yii::$app->option->website_url)) {
            yii::$app->params['site']['url'] = yii::$app->option->website_url;
        }
        ...
        if (!empty(yii::$app->option->smtp_host) && !empty(yii::$app->option->smtp_username)) {
            Yii::configure(yii::$app->mailer, [
                'useFileTransport' => false,
                'transport' => [
                    'class' => 'Swift_SmtpTransport',
                    'host' => yii::$app->option->smtp_host,  //每种邮箱的host配置不一样
                    'username' => yii::$app->option->smtp_username,
                    'password' => yii::$app->option->smtp_password,
                    'port' => yii::$app->option->smtp_port,
                    'encryption' => yii::$app->option->smtp_encryption,

                ],
                'messageConfig' => [
                    'charset' => 'UTF-8',
                    'from' => [yii::$app->option->smtp_username => yii::$app->option->smtp_nickname]
                ],
            ]);
        }
